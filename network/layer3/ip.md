...
どこまでをサブネット番号にするか？という話です。

サブネットワークはあくまで内部のネットワークとなるため、ルールはなく各ネットワークの管理者が決めます。

管理するネットワークのホスト数に応じてサブネットを切ることになりますが、ホストには最低2bit残す必要があります。

それは、全てが0のネットワークアドレスと、全てが1のブロードキャストアドレスが必要になるためです。

### サブネットマスク
「ここまでがサブネット番号だよ！」と表記する方法。IPアドレスと同じく32bit表記となる。

ネットワーク番号サブネット番号を1bit、ホスト番号を0にして表現する。

```
local-mesh1neko:~$ ipconfig getoption en0 subnet_mask
255.255.255.0
```

## IPアドレスの静的・動的割り当て
ネットワークにノードが接続されたとき、IPアドレスはどう決定するか？という考え方。

いずれも管理者から割り当てる点は変わらないが、ユーザが手動で設定する点と、自動で設定してもらう点が異なる。

基本的に自動に設定させるのが一般的で、DHCPの名で知られているものです。

実際に初めて接続する場合、自分のMACアドレスはわかっても（NICを取り付け段階で認識される）、

送信する先についての情報は全然わかりません。ブロードキャストを使っていい感じにしてるはず。
